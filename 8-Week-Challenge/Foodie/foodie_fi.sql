...
#Selecting the following random customer_id's from the subscriptions table to view their 
#onboarding journey. Checking the following customer_id's : 1,21,73,87,99,193,290,400
#Customer 1
...
total AS (SELECT COUNT(DISTINCT customer_id) AS total_customers
FROM subscriptions)
...
WITH first_plan AS (SELECT customer_id,plan_id,
...
FROM (SELECT customer_id, MIN(start_date) AS join_date
FROM subscriptions
GROUP BY customer_id) j
JOIN (SELECT customer_id, MIN(start_date) AS annual_date
FROM subscriptions
WHERE plan_id = 3
GROUP BY customer_id) a ON j.customer_id = a.customer_id;

#10)Can you further breakdown this average value into 30 day periods (i.e. 0-30 days, 31-60 days etc)
WITH next_plan_cte AS
  (SELECT *,lead(start_date, 1) over(PARTITION BY customer_id ORDER BY start_date) AS next_plan_start_date,
	lead(plan_id, 1) over(PARTITION BY customer_id ORDER BY start_date) AS next_plan
   FROM subscriptions),window_details_cte AS
  (SELECT *,datediff(next_plan_start_date, start_date) AS days,
